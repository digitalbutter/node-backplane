describe("memoryMessageStore", function(){
    var MMS = require("memoryMessageStore.js");

    describe("Store class", function(){
        var Store = MMS.Store, store;
        beforeEach(function(){
            store = new Store();
        });

        describe("test addBus function",function(){
            describe("add a single valid bus", function(){
                var bus;
                beforeEach(function(){
                    store.addBus("valid_bus");
                    bus = store.content["valid_bus"];
                });
                ...
                describe("test addChannel function",function(){
                    var channel;
                    beforeEach(function(){
                        spyOn(MMS,'Channel');
                        store.addChannel("valid_bus","valid_channel");
                        channel = bus["valid_channel"];
                    });

                    it("should have created a new channel object",function(){
                       expect(MMS.Channel).toHaveBeenCalledWith("valid_channel");
                    });

                    it("should have set the new channel in the bus",function(){
                        expect(channel).toBeTruthy();
                    });

//                    describe("add data to a channel",function(){
//                        describe("pushing a first message",function(){
//                            beforeEach({
//                                store.pushMessage("valid_bus","valid_channel","I am Content #1");
//                            });
//
